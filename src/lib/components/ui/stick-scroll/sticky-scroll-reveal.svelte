...
<div
	bind:this={ref}
	class="potato relative flex h-full justify-center space-x-10 overflow-y-auto rounded-md border-[#f6f3d8] p-10 transition ease-in-out"
>
		...
		<div class="max-w-2xl">
			{#each content as item, index (item.title + index)}
				<div class="my-20">
					...
					<p
						style="opacity: {activeCard === index ? 1 : 0.3}"
						class="text-kg mt-10 max-w-sm text-slate-900"
					>
						{item.description}
					</p>
				</div>
	...
	<div
		...
		class="sticky top-1/4 hidden h-60 w-80 overflow-hidden rounded-md bg-white lg:block"
	></div>
</div>
